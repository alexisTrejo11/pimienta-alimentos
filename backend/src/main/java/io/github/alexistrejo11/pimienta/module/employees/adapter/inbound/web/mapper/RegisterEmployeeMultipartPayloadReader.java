 ...
 */
@Component
public final class RegisterEmployeeMultipartPayloadReader {
  ...
  static {
    try {
      READ_AND_VALIDATE =
          RegisterEmployeeMultipartPayloadReader.class.getDeclaredMethod(
              "readAndValidate", MultipartFile.class);
  ...
  public RegisterEmployeeMultipartPayloadReader(ObjectMapper objectMapper, Validator mvcValidator) {
   ...
   */
  public RegisterEmployeeRequest readAndValidate(MultipartFile employeePayload) throws MethodArgumentNotValidException {
    if (employeePayload.isEmpty()) {
      throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "employee multipart part is empty");
    }
    RegisterEmployeeRequest parsed;
    try {
      parsed = objectMapper.readValue(employeePayload.getInputStream(), RegisterEmployeeRequest.class);
    } catch (IOException e) {
