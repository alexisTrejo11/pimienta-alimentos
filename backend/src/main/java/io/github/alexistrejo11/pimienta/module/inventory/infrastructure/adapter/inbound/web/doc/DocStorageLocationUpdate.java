...
/** {@code PUT /api/v1/inventory/locations/{id}} */
@Target(ElementType.METHOD)
@Retention(RetentionPolicy.RUNTIME)
@Documented
@DocJwtSecured
@Parameter(name = "id", in = ParameterIn.PATH, required = true, example = "2")
@Operation(summary = "Update storage location", description = "JSON **StorageLocationUpdateRequest**. Rate limit: **SENSITIVE_OPERATIONS**.")
@RequestBody(
    required = true,
    content =
        @Content(
            mediaType = "application/json",
            schema = @Schema(implementation = StorageLocationUpdateRequest.class)))
@ApiResponse(
    responseCode = "200",
    content =
        @Content(
...
public @interface DocStorageLocationUpdate {}
